...
if ($result.Success) {
    ...
    Write-Info "Usuario activo" ($result.Data.activeUser.username -or "NULL")
...
if ($result.Success) {
    ...
    if ($result.Data.activeUser) {
        Write-Info "Usuario activo" "$($result.Data.activeUser.username) ❌"
        Write-Fail "PC debería estar sin usuario activo"
    }
    else {
        Write-Info "Usuario activo" "NULL ✓"
...
if ($stepsFailed -eq 0) {
    ...
    Write-Host "⚠️  TEST COMPLETADO CON $stepsFailed ERRORES" -ForegroundColor Yellow
}
Write-Host ""
